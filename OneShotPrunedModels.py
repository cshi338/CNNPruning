...
for sparsityRatio in sparsityRatios:
  print("==> Peforming One-Shot Pruning with Sparsity Ratio of: " + str(sparsityRatio))
  # Model
  ...
  prune.global_unstructured(
      ...
      amount=sparsityRatio,
  )
  print("==> List Sparsity of Individual Layers..")
  globalSparsity = 0.0
  globalSparsityTotal = 0.0
  #Check sparsity of individual layers
  for x in range(0,len(parameters_to_prune)):
    if parameters_to_prune[x][1] == 'weight':
      print("Sparsity in" + layersNames[x] + ".weight: {:.2f}%".format(100. * float(torch.sum(parameters_to_prune[x][0].weight == 0)) / float(parameters_to_prune[x][0].weight.nelement())))
      ...
      print("Sparsity in" + layersNames[x] + ".bias: {:.2f}%".format(100. * float(torch.sum(parameters_to_prune[x][0].bias == 0)) / float(parameters_to_prune[x][0].bias.nelement())))
